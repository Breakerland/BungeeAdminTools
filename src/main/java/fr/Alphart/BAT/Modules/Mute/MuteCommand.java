...
public class MuteCommand extends CommandHandler {
	...
	public static class MuteCmd extends BATCommand {
		public MuteCmd() {
			super("lmute", "<player> [server] [reason]", "Mute the player on username basis on the specified server permanently or until unbanned.", Action.MUTE.getPermission());
		}
	...
	public static class MuteIPCmd extends BATCommand {
		public MuteIPCmd() {
			super("lmuteip", "<player/ip> [server] [reason]", "Mute player on an IP basis on the specified server permanently or until unbanned. No player logged in with that IP will be able to speak.", Action.MUTEIP.getPermission());
		}
	...
	public static class GMuteCmd extends BATCommand {
		public GMuteCmd() {
			super("mute", "<name> [reason]", "Mute the player on username basis on all servers (the whole network) permanently or until unbanned.", Action.MUTE.getPermission() + ".global");
		}
	...
	public static class GMuteIPCmd extends BATCommand {
		public GMuteIPCmd() {
			super("muteip", "<player/ip> [reason]", "Mute player on an IP basis on all servers (the whole network) permanently or until unbanned. No player logged in with that IP will be able to speak.", Action.MUTEIP.getPermission() + ".global");
		}
	...
	public static void handleMuteCommand(final BATCommand command, final boolean global, final boolean ipMute, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		checkArgument(!reason.equalsIgnoreCase(IModule.NO_REASON) || !BAT.getInstance().getConfiguration().isMustGiveReason(), _("noReasonInCommand"));
		...
		if (!Utils.validIP(target) && player == null) {
			...
			if (ipMute)
				...
				if ("0.0.0.0".equals(ip) && !confirmedCmd) {
					command.mustConfirmCommand(sender, command.getName() + " " + Joiner.on(' ').join(args), _("operationUnknownPlayer", new String[] { target }));
					return;
				}
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipMute ? Action.MUTEIP : Action.MUTE, sender, server), _("noPerm"));
	...
	public static class TempMuteCmd extends BATCommand {
		public TempMuteCmd() {
			super("ltempmute", "<player/ip> <duration> [server] [reason]", "Temporarily mute the player on username basis on from the specified server for duration.", Action.TEMPMUTE.getPermission());
		}
	...
	public static class TempMuteIPCmd extends BATCommand {
		public TempMuteIPCmd() {
			super("ltempmuteip", "<player> <duration> [server] [reason]", "Temporarily mute the player on IP basis on the specified server for duration. No player logged in with that IP will be able to speak.", Action.TEMPMUTEIP.getPermission());
		}
	...
	public static class GTempMuteCmd extends BATCommand {
		public GTempMuteCmd() {
			super("tempmute", "<player> <duration> [reason]", "Temporarily mute the player on username basis on all servers (the whole network) for duration.", Action.TEMPMUTE.getPermission() + ".global");
		}
	...
	public static class GTempMuteIPCmd extends BATCommand {
		public GTempMuteIPCmd() {
			super("tempmuteip", "<player/ip> <duration> [reason]", "Temporarily mute the player on IP basis on all servers (the whole network) for duration. No player logged in with that IP will be able to speak.", Action.TEMPMUTEIP.getPermission() + ".global");
		}
	...
	public static void handleTempMuteCommand(final BATCommand command, final boolean global, final boolean ipMute, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipMute ? Action.TEMPMUTEIP : Action.TEMPMUTE, sender, server), _("noPerm"));
		target = ip == null ? target : ip;
	...
	public static class UnmuteCmd extends BATCommand {
		public UnmuteCmd() {
			super("unmute", "<player> [server] [reason]", "Unmute the player on a username basis from the specified server.", Action.UNMUTE.getPermission());
		}
	...
	public static class UnmuteIPCmd extends BATCommand {
		public UnmuteIPCmd() {
			super("unmuteip", "<player/ip> [server] [reason]", "Unmute the player on a username basis from all servers (the whole network).", Action.UNMUTEIP.getPermission());
		}
	...
	public static class GUnmuteCmd extends BATCommand {
		public GUnmuteCmd() {
			super("gunmute", "<player> [reason]", "Unmute the player on an IP basis from the specified server.", Action.UNMUTE.getPermission() + ".global");
		}
	...
	public static class GUnmuteIPCmd extends BATCommand {
		public GUnmuteIPCmd() {
			super("gunmuteip", "<player/ip> [reason]", "Unmute the player on an IP basis from all servers (the whole network).", Action.UNMUTEIP.getPermission() + ".global");
		}
	...
	public static void handleUnmuteCommand(final BATCommand command, final boolean global, final boolean ipUnmute, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipUnmute ? Action.UNMUTEIP : Action.UNMUTE, sender, server), _("noPerm"));
		target = ip == null ? target : ip;

		final String[] formatArgs = { args[0] };

		checkArgument(mute.isMute(ip == null ? target : ip, server, true), IModule.ANY_SERVER.equals(server) ? _("notMutedAny", formatArgs) : ipUnmute ? _("notMutedIP", formatArgs) : _("notMuted", formatArgs));
