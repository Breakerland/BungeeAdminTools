...
public class KickCommand extends CommandHandler {
	...
	public static class KickCmd extends BATCommand {
		public KickCmd() {
			super("lkick", "<player> [reason]", "Kick the player from his current server to the lobby", Action.KICK.getPermission());
		}
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
			...
			checkArgument(args.length != 1 || !BAT.getInstance().getConfiguration().isMustGiveReason(), _("noReasonInCommand"));
			...
			if (player != null) {
				...
				checkArgument(pServer != null && !pServer.equals(player.getPendingConnection().getListener().getDefaultServer()), _("cantKickDefaultServer", new String[] { pName }));

				checkArgument(PermissionManager.canExecuteAction(Action.KICK, sender, player.getServer().getInfo().getName()), _("noPerm"));

				checkArgument(!PermissionManager.isExemptFrom(Action.KICK, pName), _("isExempt"));

				final String returnedMsg = kick.kick(player, sender.getName(), args.length == 1 ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
				...
				returnedMsg = kick.kickSQL(pUUID, RedisBungee.getApi().getServerFor(pUUID).getName(), sender.getName(), args.length == 1 ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
	...
	public static class GKickCmd extends BATCommand {
		public GKickCmd() {
			super("kick", "<player> [reason]", "Kick the player from the network", Action.KICK.getPermission() + ".global");
		}
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
			...
			if (BAT.getInstance().getRedis().isRedisEnabled()) {
				...
				final String returnedMsg;
				if (player != null)
					returnedMsg = kick.gKick(player, sender.getName(), args.length == 1 ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
				else {
					returnedMsg = kick.gKickSQL(pUUID, sender.getName(), args.length == 1 ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
				...
				final String returnedMsg = kick.gKick(player, sender.getName(), args.length == 1 ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
