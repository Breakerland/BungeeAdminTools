...
public class CoreCommand extends BATCommand {
	...
	public CoreCommand(final Core coreModule) {
		...
		subCmd = new HashMap<>();
		CREDIT = TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&9Bungee&fAdmin&cTools&a Version {version}&e - Developed by &aAlphart").replace("{version}", BAT.getInstance().getDescription().getVersion()));
		...
		HELP_MSG = TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&eType &6" + (simpleAliasesCommands.get("help") ? "/help" : "/bat help") + "&e to get help"));
	}
				...
				try {
					...
					case "ban":
						...
						if (!bans.isEmpty())
							...
							message.add(BAT.__(!Utils.validIP(entity) ? "&eThe player &a" + entity + "&e has never been banned." : "&eThe IP &a" + entity + "&e has never been banned."));
						}
						break;
					case "mute":
						...
						if (!mutes.isEmpty())
							...
							message = new ArrayList<>();
							message.add(BAT.__(!Utils.validIP(entity) ? "&eThe player &a" + entity + "&e has never been muted." : "&eThe IP &a" + entity + "&e has never been muted."));
						}
						break;
					case "kick":
						...
						if (!kicks.isEmpty())
							...
							message = new ArrayList<>();
							message.add(BAT.__(!Utils.validIP(entity) ? "&eThe player &a" + entity + "&e has never been kicked." : "&eThe IP &a" + entity + "&e has never been kicked."));
						}
						break;
					case "comment":
						...
						if (!comments.isEmpty())
							...
							message = new ArrayList<>();
							message.add(BAT.__(!Utils.validIP(entity) ? "&eThe player &a" + entity + "&e has no comment about him." : "&eThe IP &a" + entity + "&e has no comment."));
						}
						break;
					default:
	...
	public static class StaffLookupCmd extends BATCommand {
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
			final String entity = args[0];
			if (args.length == 1)
				for (final BaseComponent[] msg : LookupCmd.getLookupFormatter().getSummaryStaffLookup(entity, sender.hasPermission(Action.LOOKUP.getPermission() + ".displayip")))
					sender.sendMessage(msg);
			if (args.length > 1) {
				int page = 1;
				if (args.length > 2)
	...
	public static class ImportCmd extends BATCommand {
		...
		public ImportCmd() {
			super("import", "<" + Joiner.on('/').join(importers.keySet()) + ">", "Imports ban data from the specified source. Available sources : &a" + Joiner.on("&e,&a").join(importers.keySet()), "bat.import");
		}
			...
			if (importer != null) {
				...
				importer.startImport(new ProgressCallback<Importer.ImportStatus>() {
					...
					public void done(ImportStatus result, Throwable throwable) {
						if (throwable != null) {
							if (throwable instanceof RuntimeException) {
								sender.sendMessage(BAT.__("An error (" + throwable.getMessage() + ") has occured during the import. Please check the logs"));
								throwable.printStackTrace();
							} else {
								sender.sendMessage(BAT.__("An error has occured during the import. Please check the logs"));
								BAT.getInstance().getLogger().severe("An error has occured during the import of data from " + source + ". Please report this :");
								throwable.printStackTrace();
							}
						} else
							sender.sendMessage(BAT.__("Congratulations, the migration is finished. &a" + result.getConvertedEntries() + " entries&e were converted successfully."));
						BAT.getInstance().getModules().loadModules();
					...
					public void onProgress(ImportStatus progressStatus) {
						sender.sendMessage(BAT.__("&a" + new DecimalFormat("0.00").format(progressStatus.getProgressionPercent()) + "%&e entries converted !&a" + progressStatus.getRemainingEntries() + "&e remaining entries on a total of &6" + progressStatus.getTotalEntries()));
					}
				...
				throw new IllegalArgumentException("The specified source is incorrect. Available sources : &a" + Joiner.on("&e,&a").join(importers.keySet()));
		}
	}
	...
	public static class BackupCmd extends BATCommand {
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
			if (DataSourceHandler.isSQLite())
				throw new IllegalArgumentException("You can't backup an SQLite database with this command. " + "To save an SQLite database just copy and paste the file 'bat_database.db'.");
	...
	public static class MigrateCmd extends BATCommand {
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
			...
			ProxyServer.getInstance().getPluginManager().dispatchCommand(sender, (!isImportSimpleAlias ? "bat " : "") + "import BATSQLite");
		}
