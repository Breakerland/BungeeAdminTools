...
public class LookupFormatter {
	...
	public List<BaseComponent[]> getSummaryLookupPlayer(final String pName, final boolean displayIP) {
		...
		} else if (ProxyServer.getInstance().getPlayer(pName) != null)
			connection_state = _("connectionStateOnline").replace("{server}", ProxyServer.getInstance().getPlayer(pName).getServer().getInfo().getName());
		else
			connection_state = _("connectionStateOffline");

		final String joinChar = "&f, &3";
		final String ban_servers = !banServers.isEmpty() ? Joiner.on(joinChar).join(banServers).toLowerCase() : _("none");
		final String banip_servers = !banIPServers.isEmpty() ? Joiner.on(joinChar).join(banIPServers).toLowerCase() : _("none");
		final String mute_servers = !muteServers.isEmpty() ? Joiner.on(joinChar).join(muteServers).toLowerCase() : _("none");
		final String muteip_servers = !muteIPServers.isEmpty() ? Joiner.on(joinChar).join(muteIPServers).toLowerCase() : _("none");

		final String first_login = pDetails.getFirstLogin() != EntityEntry.noDateFound ? Core.defaultDF.format(new Date(pDetails.getFirstLogin().getTime())) : _("unknownDate");
		final String last_login = pDetails.getLastLogin() != EntityEntry.noDateFound ? Core.defaultDF.format(new Date(pDetails.getLastLogin().getTime())) : _("unknownDate");
		final String last_ip = !"0.0.0.0".equals(pDetails.getLastIP()) ? displayIP ? pDetails.getLastIP() : _("hiddenIp") : _("unknownIp");
		...
		if (ProxyServer.getInstance().getConfig().isOnlineMode())
			try {
				...
				BAT.getInstance().getLogger().severe("An error occured while fetching " + pName + "'s name history from mojang servers." + "Please report this : ");
				e.printStackTrace();
			}
		else
		...
		try {
			...
			for (final CommentEntry comm : pDetails.getComments()) {
				last_comments += _("commentRow", new String[] { String.valueOf(comm.getID()), comm.getType() == Type.NOTE ? "&eComment" : "&cWarning", comm.getContent(), comm.getFormattedDate(), comm.getAuthor() });
				i++;
				if (i == 3)
					break;
			}
		...
		final List<BaseComponent[]> finalMessage = FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', lookupPattern.replace("{connection_state}", connection_state).replace("{ban_servers}", ban_servers).replace("{banip_servers}", banip_servers).replace("{mute_servers}", mute_servers).replace("{muteip_servers}", muteip_servers).replace("{first_login}", first_login).replace("{last_login}", last_login).replace("{last_ip}", last_ip).replace("{bans_number}", String.valueOf(bansNumber)).replace("{mutes_number}", String.valueOf(mutesNumber)).replace("{kicks_number}", String.valueOf(kicksNumber)).replace("{comments_number}", String.valueOf(commentsNumber)).replace("{name_history_list}", name_history_list).replaceAll("\\{last_comments:\\d\\}", last_comments).replace("{player}", pName).replace("{uuid}", Core.getUUID(pName))
	...
	public List<BaseComponent[]> getSummaryLookupIP(final String ip) {
		...
		final String joinChar = "&f, &3";
		final String ip_users = !ipDetails.getUsers().isEmpty() ? Joiner.on(joinChar).join(ipDetails.getUsers()) : _("none");
		final String ban_servers = !banServers.isEmpty() ? Joiner.on(joinChar).join(banServers).toLowerCase() : _("none");
		final String mute_servers = !muteServers.isEmpty() ? Joiner.on(joinChar).join(muteServers).toLowerCase() : _("none");

		String replacedString = _("ipLookup").replace("{ban_servers}", ban_servers).replace("{mute_servers}", mute_servers).replace("{bans_number}", String.valueOf(bansNumber)).replace("{mutes_number}", String.valueOf(mutesNumber)).replace("{ip}", ip).replace("{ip_users}", ip_users)
				// '¤' is used as a space character, so we replace it with space and display correctly the escaped one
				.replace("¤", " ").replace("\\¤", "¤");
		...
		if (replacedString.contains("{ip_location}")) {
			...
			try {
				...
				BAT.getInstance().getLogger().log(Level.SEVERE, "Error while fetching ip location from the API. Please report this :", e);
				ipLocation = "unresolvable ip location. Check your logs";
			}
			replacedString = replacedString.replace("{ip_location}", ipLocation);
		}

		final List<BaseComponent[]> finalMessage = FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', replacedString));
	...
	public List<BaseComponent[]> getSummaryStaffLookup(final String staff, final boolean displayID) {
		...
		final List<BaseComponent[]> finalMessage = FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', _("staffLookup").replace("{bans_number}", String.valueOf(bans_number)).replace("{unbans_number}", String.valueOf(unbans_number)).replace("{mutes_number}", String.valueOf(mutes_number)).replace("{unmutes_number}", String.valueOf(unmutes_number)).replace("{kicks_number}", String.valueOf(kicks_number)).replace("{comments_number}", String.valueOf(comments_number)).replace("{warnings_number}", String.valueOf(warnings_number)).replace("{staff}", staff).replace("{uuid}", Core.getUUID(staff)).replace("¤", " ").replace("\\¤", "¤")));

		return finalMessage;
	}

	public List<BaseComponent[]> formatBanLookup(final String entity, final List<BanEntry> bans, int page, final boolean staffLookup) throws InvalidModuleException {
		...
		msg.append(lookupHeader.replace("{entity}", entity).replace("{module}", "Ban").replace("{page}", page + "/" + totalPages));
		...
		if (isBan) {
			...
			while (it.hasNext()) {
				...
				msg.append("\n");
				if (staffLookup)
					msg.append(_("activeStaffBanLookupRow", new String[] { ban.getEntity(), begin, server, reason, end }));
				else
					msg.append(_("activeBanLookupRow", new String[] { begin, server, reason, ban.getStaff(), end }));
				it.remove();
			}
		}
		...
		if (!bans.isEmpty()) {
			...
			for (final BanEntry ban : bans) {
				...
				msg.append("\n");
				if (staffLookup)
					msg.append(_("archiveStaffBanLookupRow", new String[] { ban.getEntity(), begin, server, reason, endDate, unbanReason, unbanStaff }));
				else
					msg.append(_(staffLookup ? "archiveStaffBanLookupRow" : "archiveBanLookupRow", new String[] { begin, server, reason, ban.getStaff(), endDate, unbanReason, unbanStaff }));

			}
		}

		msg.append(lookupFooter.replace("{entity}", entity).replace("{module}", "Ban").replace("{page}", page + "/" + totalPages));

		return FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', msg.toString()));
	}

	public List<BaseComponent[]> formatMuteLookup(final String entity, final List<MuteEntry> mutes, int page, final boolean staffLookup) throws InvalidModuleException {
		final StringBuilder msg = new StringBuilder();
		...
		msg.append(lookupHeader.replace("{entity}", entity).replace("{module}", "Mute").replace("{page}", page + "/" + totalPages));
		...
		if (isMute) {
			...
			while (it.hasNext()) {
				...
				msg.append("\n");
				if (staffLookup)
					msg.append(_("activeStaffMuteLookupRow", new String[] { mute.getEntity(), begin, server, reason, end }));
				else
					msg.append(_("activeMuteLookupRow", new String[] { begin, server, reason, mute.getStaff(), end }));
				it.remove();
			}
		}
		...
		if (!mutes.isEmpty()) {
			...
			for (final MuteEntry mute : mutes) {
				...
				msg.append("\n");
				if (staffLookup)
					msg.append(_("archiveStaffMuteLookupRow", new String[] { mute.getEntity(), begin, server, reason, unmuteDate, unmuteReason, unmuteStaff }));
				else
					msg.append(_("archiveMuteLookupRow", new String[] { begin, server, reason, mute.getStaff(), unmuteDate, unmuteReason, unmuteStaff }));
			}
		}

		msg.append(lookupFooter.replace("{entity}", entity).replace("{module}", "Mute").replace("{page}", page + "/" + totalPages));

		return FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', msg.toString()));
	}

	public List<BaseComponent[]> formatKickLookup(final String entity, final List<KickEntry> kicks, int page, final boolean staffLookup) throws InvalidModuleException {
		final StringBuilder msg = new StringBuilder();
		...
		msg.append(lookupHeader.replace("{entity}", entity).replace("{module}", "Kick").replace("{page}", page + "/" + totalPages));
		...
		for (final KickEntry kick : kicks) {
			...
			msg.append("\n");
			if (staffLookup)
				msg.append(_("kickStaffLookupRow", new String[] { kick.getEntity(), date, server, reason }));
			else
				msg.append(_("kickLookupRow", new String[] { date, server, reason, kick.getStaff() }));
		}

		msg.append(lookupFooter.replace("{entity}", entity).replace("{module}", "Kick").replace("{page}", page + "/" + totalPages));

		return FormatUtils.formatNewLine(ChatColor.translateAlternateColorCodes('&', msg.toString()));
	}

	public List<BaseComponent[]> commentRowLookup(final String entity, final List<CommentEntry> comments, int page, final boolean staffLookup) throws InvalidModuleException {
		{
			final StringBuilder msg = new StringBuilder();
			...
			msg.append(lookupHeader.replace("{entity}", entity).replace("{module}", "Comment").replace("{page}", page + "/" + totalPages));
			...
			for (final CommentEntry comm : comments) {
				msg.append("\n");
				if (staffLookup)
					msg.append(_("commentStaffRow", new String[] { String.valueOf(comm.getID()), comm.getType() == Type.NOTE ? "&eComment" : "&cWarning", comm.getEntity(), comm.getContent(), comm.getFormattedDate() }));
				else
					msg.append(_("commentRow", new String[] { String.valueOf(comm.getID()), comm.getType() == Type.NOTE ? "&eComment" : "&cWarning", comm.getContent(), comm.getFormattedDate(), comm.getAuthor() }));
			}

			msg.append(lookupFooter.replace("{entity}", entity).replace("{module}", "Comment").replace("{page}", page + "/" + totalPages));
