...
public class BanCommand extends CommandHandler {
	...
	public static class BanCmd extends BATCommand {
		public BanCmd() {
			super("lban", "<player> [server] [reason]", "Ban the player on username basis on the specified server permanently or until unbanned.", Action.BAN.getPermission());
		}
	...
	public static class BanIPCmd extends BATCommand {
		public BanIPCmd() {
			super("lbanip", "<player/ip> [server] [reason]", "Ban player on an IP basis on the specified server permanently or until unbanned. ", Action.BANIP.getPermission());
		}
	...
	public static class GBanCmd extends BATCommand {
		public GBanCmd() {
			super("ban", "<player> [reason]", "Ban the player on username basis on all servers (the whole network) permanently or until unbanned.", Action.BAN.getPermission() + ".global");
		}
	...
	public static class GBanIPCmd extends BATCommand {
		public GBanIPCmd() {
			super("banip", "<player/ip> [reason]", "Ban player on an IP basis on all servers (the whole network) permanently or until unbanned.", Action.BANIP.getPermission() + ".global");
		}
	...
	public static void handleBanCommand(final BATCommand command, final boolean global, final boolean ipBan, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		if (BAT.getInstance().getRedis().isRedisEnabled()) {
			...
			if (tempUUID != null && RedisBungee.getApi().isPlayerOnline(tempUUID))
				pUUID = tempUUID;
		}
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipBan ? Action.BANIP : Action.BAN, sender, server), _("noPerm"));
	...
	public static class TempBanCmd extends BATCommand {
		public TempBanCmd() {
			super("ltempban", "<player> <duration> [server] [reason]", "Temporarily ban the player on username basis on from the specified server for duration.", Action.TEMPBAN.getPermission());
		}
	...
	public static class TempBanIPCmd extends BATCommand {
		public TempBanIPCmd() {
			super("ltempbanip", "<player/ip> <duration> [server] [reason]", "Temporarily ban the player on IP basis on the specified server for duration.", Action.TEMPBANIP.getPermission());
		}
	...
	public static class GTempBanCmd extends BATCommand {
		public GTempBanCmd() {
			super("tempban", "<player> <duration> [reason]", "Temporarily ban the player on username basis on all servers (the whole network) for duration.", Action.TEMPBAN.getPermission() + ".global");
		}
	...
	public static class GTempBanIPCmd extends BATCommand {
		public GTempBanIPCmd() {
			super("tempbanip", "<player/ip> <duration> [reason]", "Temporarily ban the player on IP basis on all servers (the whole network) for duration.", Action.TEMPBANIP.getPermission() + ".global");
		}
	...
	public static void handleTempBanCommand(final BATCommand command, final boolean global, final boolean ipBan, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipBan ? Action.TEMPBANIP : Action.TEMPBAN, sender, server), _("noPerm"));
		target = ip == null ? target : ip;
	...
	public static class UnbanCmd extends BATCommand {
		public UnbanCmd() {
			super("unban", "<player> [server] [reason]", "Unban the player on a username basis from the specified server.", Action.UNBAN.getPermission(), "pardon");
		}
	...
	public static class UnbanIPCmd extends BATCommand {
		public UnbanIPCmd() {
			super("unbanip", "<player/ip> [server] [reason]", "Unban IP from the specified server", Action.UNBANIP.getPermission(), "pardonip");
		}
	...
	public static class GUnbanCmd extends BATCommand {
		public GUnbanCmd() {
			super("gunban", "<player> [reason]", "Unban the player on a username basis from all servers (the whole network).", Action.UNBAN.getPermission() + ".global", "gpardon");
		}
	...
	public static class GUnbanIPCmd extends BATCommand {
		public GUnbanIPCmd() {
			super("gunbanip", "<player/ip> [reason]", "Unban the player on an IP basis from all servers (the whole network).", Action.UNBANIP.getPermission() + ".global", "gpardonip");
		}
	...
	public static void handlePardonCommand(final BATCommand command, final boolean global, final boolean ipUnban, final CommandSender sender, final String[] args, final boolean confirmedCmd) {
		...
		if (!global)
			checkArgument(PermissionManager.canExecuteAction(ipUnban ? Action.UNBANIP : Action.UNBAN, sender, server), _("noPerm"));
		target = ip == null ? target : ip;

		final String[] formatArgs = { args[0] };

		checkArgument(ban.isBan(ip == null ? target : ip, server), IModule.ANY_SERVER.equals(server) ? _("notBannedAny", formatArgs) : ipUnban ? _("notBannedIP", formatArgs) : _("notBanned", formatArgs));
