...
public class DataSourceHandler {
	...
	public DataSourceHandler(final String host, final String port, final String database, final String username, final String password) throws SQLException {
		...
		ds.setJdbcUrl("jdbc:mysql://" + this.host + ":" + this.port + "/" + this.database + "?useLegacyDatetimeCode=false&serverTimezone=" + TimeZone.getDefault().getID());
	...
	public DataSourceHandler() {
		/*
		 * As SQLite supports concurrency pretty badly (locked database which causes problem), we're gonna get a connection from the DriverManager each time we need to acces to the database. In the
		 * contrary of BoneCP with mysql in which we saved connection to optimize perfomance, it's not necessary with SQLite. FYI, here are the results of test : execute 1000 insert request using
		 * SQLite, with or without using the same connection : - Using the same connection it took : 22820 ms - Getting another connection each time (DriverManager.getConnection), it took : 24186 ms
		 * The difference is only 1366 ms for 1000 request, that means on average additional 1.3 ms, which is insignificant as we are executing almost every query async. To the people who read that,
		 * all these calculations can seem a little overrated, but I really like to improve perfomance at the most and I'm pretty curious :p
		 */
		sqlite = true;
		try {
			SQLiteConn = DriverManager.getConnection("jdbc:sqlite:" + BAT.getInstance().getDataFolder().getAbsolutePath() + File.separator + "bat_database.db");
			SQLiteConn.close();
	 ...
	 *             if MySQL is not used or if the creation of the backup file failed */
	public void generateMysqlBackup(final Callback<String> onComplete) throws RuntimeException {
		ProxyServer.getInstance().getScheduler().runAsync(BAT.getInstance(), new Runnable() {
			...
			public void run() {
				...
				final File backupDirectory = new File(BAT.getInstance().getDataFolder().getAbsolutePath() + File.separator + "databaseBackups");
				backupDirectory.mkdir();
				File backupFile = new File(backupDirectory.getAbsolutePath() + File.separator + "backup" + new SimpleDateFormat("dd-MMM-yyyy_HH'h'mm").format(Calendar.getInstance().getTime()) + ".sql");
				for (int i = 0;; i++)
				...
				final String tables = Joiner.on(' ').join(Arrays.asList(SQLQueries.Ban.table, SQLQueries.Mute.table, SQLQueries.Kick.table, SQLQueries.Comments.table, SQLQueries.Core.table));
				String backupPath = backupFile.getAbsolutePath();
				if (backupPath.contains(" "))
					backupPath = "\"" + backupPath + "\"";
				backupCmd = backupCmd.replace("{user}", username).replace("{database}", database).replace("{path}", backupPath).replace("{tables}", tables);
				if (password.equals(""))
